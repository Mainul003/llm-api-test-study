...
\author{
  \IEEEauthorblockN{Md.\ Mainul Islam}
  \IEEEauthorblockA{
    \textit{WebAlive (Web Mascot Pty Ltd)} \\
    Melbourne, Australia (Remote) \\
    \textit{Dept.\ of CSE, United International University}\\
    Dhaka, Bangladesh \\
    mainulfuad95@gmail.com
  }
  \and
  \IEEEauthorblockN{A.K.M.\ Muzahidul Islam}
  \IEEEauthorblockA{
    \textit{Dept.\ of CSE}\\
...
Drawing on five years of industrial API testing experience, we discuss the
complementary potential of LLMs and practitioners in building robust API test
suites, and release a replication package at
\texttt{https://github.com/Mainul003/llm-api-test-study}.
\end{abstract}

\begin{IEEEkeywords}
REST API testing, large language models, test generation, empirical study,
software quality assurance, OpenAPI
\end{IEEEkeywords}
...
assertions. Critically, \emph{all} of these studies focus on function-level
unit tests, not REST API tests.
...
maximize code coverage. Kim et al.~\cite{Kim2022} empirically compared
these black-box tools, concluding that operation coverage and fault detection
...
REST API tests to developer-written tests}---the industrial baseline.
Understanding this gap is essential: if LLMs can match or exceed developer
tests on key quality metrics, they offer a practical path to reducing manual
testing effort; if they fall short in specific dimensions (e.g., stateful
scenarios, authentication), practitioners need to know where human expertise
remains irreplaceable. This paper addresses that question.
...
All experiments were conducted on Ubuntu 24.04 LTS, 16\,GB RAM. API services were deployed locally using Docker
where images were available; for projects without Docker configurations we
used the publicly hosted demo servers. Newman~CLI was invoked with a 30-second
per-request timeout. The LLM API was queried at temperature~0 with a maximum
of 4,096 output tokens per call.
...
Table~\ref{tab:rq1} shows per-project endpoint coverage for both test suites.
...
Table~\ref{tab:rq2} reports DDR across 40 $\times$ 3 mutants.
...
Several APIs require operations to be executed in order~(e.g., \texttt{POST
/cart} before \texttt{POST /checkout}). LLMs generated independent test cases
for each endpoint, ignoring producer-consumer dependencies. This accounts for
24\% of endpoints where LLM-generated tests could not be executed
as-is.
...
Our findings motivate several future research directions. First, improving LLM
handling of authentication flows---for example, by incorporating token
provisioning scripts into the generation prompt---could substantially increase
the practical executability of generated tests~(F1). Second, integrating
producer-consumer dependency inference~(as in RESTler~\cite{Atlidakis2019} and
AutoRestTest~\cite{Stennett2025}) into LLM prompting strategies could address
the stateful sequencing gap~(F2). Third, grounding LLM generation in
requirements documents alongside API specifications, as proposed by
APITestGenie~\cite{Pereira2024}, may close the business-rule constraint gap~(F3).
...
observed in the project issue trackers of our dataset subjects.
...
This work was supported by the Department of CSE, United International University, Bangladesh.
...
``Design Choices Made by LLM-Based Test Generators Prevent Them from Finding
Bugs,''
\textit{arXiv preprint arXiv:2412.14137}, 2024.

\bibitem{Martin2021}
A.~Martin-Lopez, S.~Segura, and A.~Ruiz-Cort\'{e}s,
``RESTest: Black-Box Constraint-Based Testing of RESTful Web APIs,''
in \textit{Proc. ACM ISSTA}, pp.~459--471, 2021.
